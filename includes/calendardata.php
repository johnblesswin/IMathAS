...
function getCalendarEventData($cid, $userid, $stuview = false) {
	...
	if (isset($itemlist['Assessment'])) {
		...
		$stm = $DBH->query("SELECT id,name,startdate,enddate,reviewdate,reqscore,reqscoreaid FROM imas_assessments WHERE avail=1 AND date_by_lti<>1 AND id IN ($typeids) AND enddate<2000000000 ORDER BY name");
