...
switch($_POST['action']) {
	...
	case "addcourse":
		...
		if ($_POST['action']=='modify') {
			...
			$query .= "allowunenroll=:allowunenroll,copyrights=:copyrights,msgset=:msgset,toolset=:toolset,theme=:theme,ltisecret=:ltisecret,istemplate=:istemplate,deftime=:deftime,deflatepass=:deflatepass,startdate=:startdate,enddate=:enddate WHERE id=:id";
			$qarr = array(':name'=>$_POST['coursename'], ':enrollkey'=>$_POST['ekey'], ':hideicons'=>$hideicons, ':available'=>$avail, ':lockaid'=>$_POST['lockaid'],
				...
				':deftime'=>$deftime, ':deflatepass'=>$deflatepass, ':startdate'=>$startdate, ':enddate'=>$enddate, ':id'=>$_GET['id']);
			if ($myrights<75) {
			...
			$query = "INSERT INTO imas_courses (name,ownerid,enrollkey,hideicons,picicons,allowunenroll,copyrights,msgset,toolset,showlatepass,itemorder,available,startdate,enddate,istemplate,deftime,deflatepass,theme,ltisecret,blockcnt) VALUES ";
			$query .= "(:name, :ownerid, :enrollkey, :hideicons, :picicons, :allowunenroll, :copyrights, :msgset, :toolset, :showlatepass, :itemorder, :available, :startdate, :enddate, :istemplate, :deftime, :deflatepass, :theme, :ltisecret, :blockcnt);";
			$stm = $DBH->prepare($query);
